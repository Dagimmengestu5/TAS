...
const JobBoard = () => {
    ...
    const filteredJobs = jobs.filter(job => {
        const title = (job.title || job.requisition?.title || '').toLowerCase();
        const dept = (job.requisition?.department?.name || '').toLowerCase();
                ...
                <header className="mb-16 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div>
                        <motion.div
                            ...
                            className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-4 inline-block"
                        >
                            Active Opportunities
                        </motion.div>
                        <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-900">Market Intelligence</h1>
                        <p className="text-gray-400 font-bold uppercase text-[9px] tracking-wider mt-2">Available Nodes for Acquisition</p>
                    </div>
                </header>
                    ...
                    <div className="flex gap-2 p-1.5 bg-white rounded-xl border border-gray-100 shadow-sm">
                        {['all', 'internal', 'external'].map((t) => (
                            <button
                                key={t}
                                onClick={() => setFilter(t)}
                                className={`px-6 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${filter === t
                        ...
                        {filteredJobs.length > 0 ? filteredJobs.map((job) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-8">
                                    ...
                                    <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider ${job.is_internal ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-50 text-blue-700'
                                        }`}>
                                ...
                                <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between">
                                    ...
                                    <div className="flex gap-2">
                                        <Link
                                            to={`/jobs/${job.id}`}
                                            className="bg-gray-50 text-black px-4 py-2.5 rounded-lg font-bold text-[9px] uppercase tracking-wider hover:bg-gray-200 transition-all flex items-center gap-2"
                                        >
                                            Inspect
                                        </Link>
                                        <Link
                                            to={`/jobs/${job.id}?apply=true`}
                                            className="bg-gray-100 text-black px-6 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider hover:bg-yellow-400 group-hover:bg-yellow-400 group-hover:text-black transition-all flex items-center gap-2"
                                        >
                            ...
                            <div className="col-span-full py-40 text-center bg-white rounded-3xl border border-dashed border-gray-100">
                                <p className="text-gray-300 font-bold text-lg uppercase tracking-wider">No matching roles detected.</p>
                            </div>
                        )}
                    </div>
                )}
