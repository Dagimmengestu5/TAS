            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-yellow italic">{companies.length} Registered Entities</span>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    >
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                            ...
                            <h3 className="text-lg font-black text-gray-900 uppercase italic tracking-tight mb-2">{company.name}</h3>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">ID: {company.id} • Registered</p>
                        </div>
                    </motion.div>
                ))}
            </div>
                    ...
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl relative overflow-hidden border border-gray-100">
                            <div className="bg-gray-950 p-8 flex items-center justify-between">
                                <h2 className="text-xl font-black text-brand-yellow uppercase tracking-tight italic">{editingCompany ? 'Modify Entity' : 'New Entity'}</h2>
                            ...
                            <form onSubmit={handleSubmit} className="p-10 space-y-8">
                                <div className="group">
                                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em] italic mb-2.5 block ml-1">Company Name</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <button type="submit" className="flex-[2] py-4 bg-gray-900 text-brand-yellow rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 italic">Execute <ChevronRight className="w-4 h-4" /></button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
